...
const SidebarFilter = ({ sort, setSort, products, items, setItems }) => {
  ...
  const setNewItems = () => {
    setItems((prev) => {
      let next = [...prev];
      const selectedBrands = brands.filter(obj => obj.checked).map(obj => obj.txt)
      const selectedsModels = models.filter(obj => obj.checked).map(obj => obj.txt)
      if (arrayControl(selectedBrands) && !arrayControl(selectedsModels)) {
        next = products.filter(obj => selectedBrands.includes(obj.brand))
        return next
      }
      else if (!arrayControl(selectedBrands) && arrayControl(selectedsModels)) {
        next = products.filter(obj => selectedsModels.includes(obj.model))
        return next
      }
      else if (arrayControl(selectedBrands) && arrayControl(selectedsModels)) {
        next = products.filter(obj => selectedBrands.includes(obj.brand) && selectedsModels.includes(obj.model))
        return next
      }
      else {
        return products
  ...
  const modelFilter = (txt) => {
    ...
    setModels(() => {
      const selectedBrandsIdies = brands.filter((obj) => obj.checked).map((obj) => obj.id);
      return allModels.filter((obj) => selectedBrandsIdies.includes(obj.parentId));
    });
  }
  return (
